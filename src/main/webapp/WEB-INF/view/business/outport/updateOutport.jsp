...
<form class="layui-form" action="#" method="get" id="frm" lay-filter="frm">
		...
		<div class="layui-form-item" style="margin-top: 20px">
			    ...
			    <div class="layui-input-block">
			      <select name="providerid" id="providerid" lay-filter="providerid">
			        ...
			        <c:forEach var="sn" items="${list }">
				        <option value="${sn.id }"  ${sn.id==outport.providerid?"selected":"" } >${sn.providername }</option>
			        </c:forEach>
			      </select>
			    </div>
			</div>
			<div class="layui-inline">
				...
				<div class="layui-input-inline">
					 <select name="goodsid" value="${outport.goodsid }" id="goodsid" lay-filter="aihao">
		...
		<div class="layui-form-item">
			...
			<div class="layui-input-inline">
				<input type="text" name="number" value="${outport.number }" id="number" lay-verify="number"
			...
			<div class="layui-inline">
				...
				<div class="layui-input-inline">
					<input type="text" name="outportprice" value="${outport.outportprice }" id="outportprice" lay-verify="number"
						autocomplete="off" class="layui-input">
				</div>
			</div>
		</div>
		<div class="layui-form-item">
				<div class="layui-inline">
					...
					<div class="layui-inline">
						<input type="radio" name="paytype" value="支付宝" title="支付宝" ${outport.paytype=="支付宝"?"checked":"" }> 
						<input type="radio" name="paytype" value="微信" title="微信" ${outport.paytype=="微信"?"checked":"" }>
						<input type="radio" name="paytype" value="银联" title="银联" ${outport.paytype=="银联"?"checked":"" }>
					</div>
				</div>
		</div>
		<div class="layui-form-item">
			...
			<div class="layui-input-block">
				<textarea rows="5" cols="80%" name="remark">${outport.goodsid }</textarea>
			</div>
		</div>
		<div class="layui-form-item">
	...
	layui.use([ 'form','jquery','layer' ], function() {
		...
		form.on("submit(outportSubmit)",function(data){
			var params = $('#frm').serialize();
			 $.post('${ctx}/outport/updateOutport.action',params,function(data){
				layer.msg(data.msg);
				parent.tablereload();
