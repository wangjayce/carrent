    ...
    var form = layui.form,
	   ...
	   laydate.render({
	    ...
	    tableIns = table.render({
	        elem: '#inportList',
	        url : '${ctx}/inport/uploadAllInport.action',
	        cellMinWidth : 95,
	        page : true,
	        height : "full-225",
	        limit : 10,
	        ...
	        cols : [[
	            ...
	            {field: 'id', title: '进货单ID', width:100, align:"center"},
	            ...
	            {field: 'inporttime', title: '进货时间', width:140, align:"center"},
	            {field: 'operateperson', title: '操作人',width:100, align:'center'},
	            {field: 'number', title: '进货数量', align:'center'},
	            {field: 'remark', title: '备注', align:'center'},
	            {title: '操作', width:170, templet:'#inportListBar',fixed:"right",align:"center"}
	        ]]
	    });
                ...
                case 'batchDelet':
                	...
                	var ids="?a=1";
                	 if(data.length > 0) {
 	                    ...
 	                    layer.confirm('确定删除选中的进货单？', {icon: 3, title: '提示信息'}, function (index) {
 	                         $.post("${ctx}/inport/deleteInport.action"+ids,function(data){
 	                        	layer.msg(data.msg);
 	                        	tableIns.reload();
 	                        	layer.close(index);
 	                         })
        ...
        function addInport(){
        	layer.open({
      		  type: 2, 
      		  title:'添加进货单',
      		  content: '${ctx}/inport/toaddInport.action', //这里content是一个URL，如果你不想让iframe出现滚动条，你还可以content: ['http://sentsin.com', 'no']
      		  area : ['800px','600px']
        	}); 
        }
        
        function updateInport(id){
        	layer.open({
      		  type: 2, 
      		  title:'修改进货单',
      		  content: '${ctx}/inport/toupdateInport.action?id='+id, //这里content是一个URL，如果你不想让iframe出现滚动条，你还可以content: ['http://sentsin.com', 'no']
      		  area : ['800px','600px']
        	}); 
        }
